...
const CarSearch: React.FC<CarSearchProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    async function loadCars() {
      ...
      try {
        ...
        if (!ignore) {
          setError(err instanceof Error ? err.message : "Nie udało się pobrać floty.");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
  ...
  const filteredCars = cars.filter((car) => {
    ...
    const matchesBranch = branch === "Wszystkie" || carBranchName(car) === branch;
    const matchesBody = bodyType === "Wszystkie" || carBodyType(car) === bodyType;
    ...
    <div className="bg-surface font-body-md text-on-surface selection:bg-primary-fixed min-h-screen pb-12">
      <main className="max-w-container-max mx-auto px-margin-desktop py-stack-lg">
        <section className="mb-stack-lg">
          <p className="font-label-md text-primary mb-unit">Flota Harvest Motion</p>
          ...
          <div className="p-stack-md rounded-[1.5rem] border border-outline-variant bg-white/80 backdrop-blur-md shadow-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-gutter">
            ...
            <button
              className="bg-secondary text-on-secondary px-8 py-3.5 rounded-xl self-end font-label-md"
              type="button"
            >
              Filtruj
            </button>
